# Layer 15 — KMS + durable backup storage. The PERSISTENT foundation that must
# outlive the compute layer (50) we destroy between iterations:
#   - the EBS CMK that encrypts node root disks + gp3 PVCs (moved here from the
#     compute module so a `tofu destroy` of 50 no longer schedules it for
#     deletion and bills through the window — ADR-0006);
#   - a separate CMK for Postgres backups, and the S3 bucket CNPG/Barman writes
#     to (so backups survive every teardown).
#
